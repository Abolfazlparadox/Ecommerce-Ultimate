...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 'تبریک {}, حساب کاربری شما با موفقیت ساخته شد .'.format(new_user))
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            ...
            messages.success(
                ...
                request, f'خوش آمدید {username}   شما با موفقیت وارد سیستم شدید')
            return redirect('accounts:dashboard_customer')

        else:
            messages.warning(request, ' نام کاربری یا کلمه عبور اشتباه وارد شده است')
...
def dashboard_customer(request):
    ...
    if request.method == 'POST':
        ...
        profile.save()
        messages.success(
            request, 'اطلاعات پروفایل کاربری شما با موفقیت ذخیره شد !')
        return redirect("accounts:dashboard_customer")
    ...
    if request.method == 'POST':
        ...
        if image:
            try:
                ...
                messages.warning(request, 'با عرض پوزش، تصویر شما نامعتبر است')
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 'رمز عبور شما با موفقیت تغییر کرد!')
            return redirect('accounts:change_password')

        else:
            messages.warning(request, 'لطفا ابتدا ارور پیش آمده را حل کنید.')
    else:
