...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['comments'] = ArticleComment.objects.filter(article_id=article.id, parent=None ,is_active=True).order_by('-create_date').prefetch_related('articlecomment_set')
        context['comments_count'] = ArticleComment.objects.filter(article_id=article.id , is_active=True).count()
        #.prefetch_related('articlecomment_set') : With this code, we no longer need to receive all the answers to 100 comments in the form of 101 commands, the code gives us all the comments.
        return context
...
def add_article_comment(request: HttpRequest):
    if request.user.is_authenticated:
        ...
        new_comment.save()
        context = {
            'comments': ArticleComment.objects.filter(article_id=article_id,is_active=True, parent=None).order_by('-create_date').prefetch_related('articlecomment_set'),
